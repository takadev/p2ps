...
class ClientCore:
    def __init__(self, my_port=50030, core_host=None, core_port=None):
        self.client_state = STATE_INIT
        print('Initializing ClientCore...')
        self.my_ip = self.__get_myip()
        print('Server IP address is set to... ', self.my_ip)
        self.my_port = my_port
        self.cm = ConnectionManager4Edge(self.my_ip, self.my_port, core_host, core_port)

    def start(self):
        self.client_state = STATE_ACTIVE
        self.cm.start()
        self.cm.connect_core_node()

    def shutdown(self):
        self.client_state = STATE_SHUTTING_DOWN
        print('Shutdown edge node...')
        self.cm.conneciton_close()
